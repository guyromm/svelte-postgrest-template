...
module.exports = {handler:async (req, res) => {
    ...
    let path = req.queryStringParameters._path?req.queryStringParameters._path:'/'; // req.url?req.url.replace('testicle/','/'):'/';
    if (!path.startsWith('/')) path='/'+path;
    //let get = qs.stringify({...req.queryStringParameters,_path:undefined});
    let get = '';
    for (let [k,v] of Object.entries(req.queryStringParameters))
    {
	if (['_path'].includes(k)) continue;
	get+=(get.length?'&':'?')+encodeURIComponent(k)+'='+encodeURIComponent(v);
    }
